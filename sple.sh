...
spSSLDir="/etc/apache-sp/vhosts.d/"
...
if [ "$theAction" == "uninstall" ]; then
	...
	access_log /srv/users/serverpilot/log/$appName/dev_apache.access.log main;
	error_log /srv/users/serverpilot/log/$appName/dev_apache.error.log;
	...
	include /etc/apache-sp/vhosts.d/$appName.d/*.nonssl_conf;
	include /etc/apache-sp/vhosts.d/$appName.d/*.conf;
	...
	sudo service apache-sp start && sudo service apache-sp reload
else
