...
```bash
$ git clone https://github.com/DavidFricker/serverpilot-letsencrypt.git && cd serverpilot-letsencrypt && sudo mv sple.sh /usr/local/bin/rwssl2 && sudo chmod +x /usr/local/bin/rwssl2 && (crontab -l ; echo "@monthly \"sudo service apache-sp stop && yes | letsencrypt --standalone renew &>/dev/null && service apache-sp start && service apache-sp reload\"")| crontab - && service cron reload
```
